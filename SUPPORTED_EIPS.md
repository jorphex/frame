...
| [EIP-3326](https://eips.ethereum.org/EIPS/eip-3326) | Implemented | `wallet_switchEthereumChain` validates a known, enabled destination, cancels untouched requests for the origin's previous chain, switches only that origin, and returns `null`. Frame intentionally does not add a second confirmation after account access: unlike the EIP's single-active-chain model, Frame routes each origin independently and keeps transaction/signature review chain-bound.                                                                                             |
| [EIP-747](https://eips.ethereum.org/EIPS/eip-747)   | Implemented | `wallet_watchAsset` supports the standard's checksummed legacy ERC-20 and ERC-1046 suggestions on connected chains and acknowledges valid requests before prompting. ERC-1046 metadata is bounded to inline JSON or IPFS through Frame's configured Kubo endpoint, reconciled with contract reads, and validated before acknowledgement; arbitrary HTTP(S) and image fetches are rejected. Other asset types require separate standards.                                                        |
| [EIP-6963](https://eips.ethereum.org/EIPS/eip-6963) | Partial     | Frame's embedded-dapp injector announces its EIP-1193 provider through immutable EIP-6963 metadata and retains the same provider as the legacy `window.ethereum` fallback. Browser-wide discovery belongs to the separate Frame companion extension and is not implemented or claimed by this repository.                                                                                                                                                                                       |
...
| [EIP-2612](https://eips.ethereum.org/EIPS/eip-2612) | Review-only | Exact EIP-712 permit declarations with signed chain and verifying-contract bindings are detected, decoded, and checked for owner, spender, amount, expiry, nonce, and unlimited authority. Frame does not implement token permit contracts.                                                             |
| [Permit2](https://github.com/Uniswap/permit2)       | Review-only | Exact allowance, transfer, batch, and witness EIP-712 declarations are summarized without changing the signed payload. Frame requires explicit consent for spend authority, maximum-width amounts, and noncanonical verifying contracts; it does not execute Permit2 or resolve token metadata.         |
| [ERC-3009](https://eips.ethereum.org/EIPS/eip-3009) | Review-only | Exact V4 transfer, receive, and cancellation authorizations with signed chain/contract bindings are summarized without changing the payload. Frame enforces the signing authorizer and explicit transfer consent; ERC-3009 remains Draft and Frame does not execute it or infer token-contract support. |
...
| [ERC-1271](https://eips.ethereum.org/EIPS/eip-1271) | Unsupported | Frame supports canonical watch-only accounts but does not detect or infer ERC-1271 or Safe support from bytecode. Contract-signature validation and Safe signing flows are not implemented; watch-only signing requests fail before submission.                                                         |
...
| [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) | Awareness-only | Frame detects the exact delegation designator through the configured RPC, shows chain-scoped account status, warns for ordinary transactions, and blocks sequential wallet-call batches from delegated accounts. Type-4 transactions and authorization lists are rejected; Frame cannot create, replace, or revoke delegation. |
